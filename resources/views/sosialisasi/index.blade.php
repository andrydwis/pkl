                    ...
                    <div class="row">
                        <div class="col-6">
                            <div class="form-group">
                                ...
                                <input id="tanggal_dari" type="date"
                                    class="form-control @error('tanggal_dari'){{ 'is-invalid' }}@enderror"
                                    name="tanggal_dari" value="{{ old('tanggal_dari') ?? '' }}" autofocus>
                                @error('tanggal_dari')
                        ...
                        <div class="col-6">
                            <div class="form-group">
                                ...
                                <input id="tanggal_sampai" type="date"
                                    class="form-control @error('tanggal_sampai'){{ 'is-invalid' }}@enderror"
                                    name="tanggal_sampai" value="{{ old('tanggal_dari') ?? '' }}">
                    ...
                    <tbody>
                        @foreach ($sosialisasis as $sosialisasi)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('data-sosialisasi.show', ['sosialisasi' => $sosialisasi]) }}"
                                        class="btn btn-primary btn-icon"><i class="fas fa-exclamation"></i></a>
                                    <a href="{{ route('data-sosialisasi.edit', ['sosialisasi' => $sosialisasi]) }}"
                                        class="btn btn-primary btn-icon"><i class="fas fa-edit"></i></a>
                                    <button class="btn btn-icon btn-danger" id="modal-destroy-{{ $sosialisasi->id }}"
                                        data-toggle="modal" data-target="#modal-destroy-{{ $sosialisasi->id }}"><i
                                            class="fas fa-trash"></i></button>
                                    <a href="{{ route('data-sosialisasi.process-view', ['sosialisasi' => $sosialisasi]) }}"
                                        class="btn btn-success btn-icon"><i class="fas fa-play"></i></a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
...
<!-- modal -->
@foreach ($sosialisasis as $sosialisasi)
    <div class="modal fade" tabindex="-1" role="dialog" id="modal-destroy-{{ $sosialisasi->id }}" aria-hidden="true"
        style="display: none;">
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Hapus Sosialisasi</h5> <button type="button" class="close"
                        data-dismiss="modal" aria-label="Close"><span><i
                ...
                <div class="modal-footer flex justify-content-center">
                    <form action="{{ route('data-sosialisasi.destroy', ['sosialisasi' => $sosialisasi]) }}"
                        method="post">
                        @csrf
                        @method('DELETE')
